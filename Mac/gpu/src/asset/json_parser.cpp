...
bool JsonValue::is_bool() const { return type_ == Type::Bool; }
...
std::int64_t JsonValue::as_int64() const { return static_cast<std::int64_t>(std::llround(as_number())); }
...
const JsonValue* JsonValue::find(const std::string& key) const {
    ...
    return iterator == object_value_.end() ? nullptr : &iterator->second;
}

...
const char* BufferKindLabel(const DecodePlanBufferKind buffer_kind) {
    switch (buffer_kind) {
        case DecodePlanBufferKind::kHiddenSlot0:
            return "hidden_slot0";
        case DecodePlanBufferKind::kHiddenSlot1:
...
std::vector<DecodePlanAccessRange> StageAccesses(const DecodePlanStage& stage,
                                                 const Qwen3Runner& model,
                                                 const KVCache& kv_cache) {
    const std::size_t hidden_bytes = model.hidden_size() * sizeof(float);
...
void RefreshDecodePlanAccesses(DecodeExecutionPlan* plan, const Qwen3Runner& model, const KVCache& kv_cache) {
    if (plan == nullptr) {
        return;
    }
...
std::unique_ptr<DecodeExecutionPlan> BuildDecodePlan(const Qwen3Runner& model, const KVCache& kv_cache) {
    auto plan = std::make_unique<DecodeExecutionPlan>();
    plan->layer_count = model.num_layers();
    plan->hidden_size = model.hidden_size();
    ...
    for (std::size_t layer_index = 0; layer_index < plan->layer_count; ++layer_index) {
        ...
        stage.batch_id = batch_id;
        tracker.Record(stage.accesses, layer_index, stage.label);
    ...
    if (!tracker.CanMerge(logits_stage.accesses)) {
        tracker.Reset();
        ++batch_id;
    }
    logits_stage.batch_id = batch_id;
    tracker.Record(logits_stage.accesses, plan->stages.size(), logits_stage.label);
...
bool Qwen3DecodePlanner::RunDecode(const MetalContext& context,
    ...
    const auto* qwen3_runner = dynamic_cast<const Qwen3Runner*>(&model);
    if (qwen3_runner == nullptr) {
        if (error_message != nullptr) {
            *error_message = "Qwen3DecodePlanner requires a Qwen3Runner";
        }
        return false;
    }
    ...
    if (kv_cache == nullptr) {
        if (error_message != nullptr) {
            *error_message = "Qwen3DecodePlanner requires a KV cache";
        }
        return false;
    }

    if (decode_plan_ != nullptr &&
        decode_plan_->layer_count == qwen3_runner->num_layers() &&
        decode_plan_->hidden_size == qwen3_runner->hidden_size() &&
        decode_plan_->vocab_size == qwen3_runner->vocab_size() &&
        ...
        RefreshDecodePlanAccesses(decode_plan_.get(), *qwen3_runner, *kv_cache);
    } else {
        decode_plan_ = BuildDecodePlan(*qwen3_runner, *kv_cache);
    }
    if (decode_plan_ == nullptr) {
        if (error_message != nullptr) {
            *error_message = "Failed to build qwen3 decode plan";
        }
        return false;
    }

    if (!EnsureHiddenBuffer(context, 0, qwen3_runner->hidden_size(), error_message) ||
        !EnsureHiddenBuffer(context, 1, qwen3_runner->hidden_size(), error_message)) {
        return false;
    }
    const TensorDesc hidden_desc = TensorDesc::CreateContiguous(DataType::kFloat32, {1, qwen3_runner->hidden_size()});
    ...
    run_stats.layer_stage_count = qwen3_runner->num_layers();
    ...
    for (std::size_t blocker_stage_index = 0; blocker_stage_index < decode_plan_->stages.size(); ++blocker_stage_index) {
        ...
        if (conflict.has_conflict) {
            ...
            stage_blocker.resource_label = BufferKindLabel(conflict.buffer_kind);
        ...
        switch (stage.kind) {
            ...
            case DecodePlanStage::Kind::kLayer: {
                ...
                const bool ok = stage.kind == DecodePlanStage::Kind::kEmbedAndFirstLayer
                    ? qwen3_runner->model().ForwardHiddenCachedRange(context,
                                                                     pipeline_cache,
                                                                     token_ids,
                                                                     kv_cache,
                                                                     ...
                                                                     stream_mode,
                                                                     error_message)
                    : qwen3_runner->model().ForwardHiddenFromStatesCachedRange(context,
                                                                               pipeline_cache,
            ...
            case DecodePlanStage::Kind::kLogits:
                if (!qwen3_runner->ForwardLogitsFromHidden(context,
                                                           pipeline_cache,
                                                           hidden_slots[stage.input_slot],
                                                           logits_output,
                                                           temporary_arena,
    ...
    last_run_stats_ = std::move(run_stats);
    return true;
}

const DecodeExecutionPlan* Qwen3DecodePlanner::decode_plan() const {
    return decode_plan_.get();
}

const DecodePlanRunStats& Qwen3DecodePlanner::last_run_stats() const {
    return last_run_stats_;
}

bool Qwen3DecodePlanner::EnsureHiddenBuffer(const MetalContext& context,
                                            std::size_t slot,
